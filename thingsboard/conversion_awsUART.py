...
def adcConversion(refV,prtVal,brgVal):
    ...
    brgRefV = 5*1318.0/(22000.0+1318.1)
  ...
  while(1):
  	for i in range(3):
		...
		while (current_sensor<7):
			...
			(T,P,R) = adcConversion(5,sensor_data['temperature'],sensor_data['water_stress'])
			(payload['prtR'],payload['brgSig'],payload['temp_logger']) = adcConversion(5,sensor_data['temperature'],sensor_data['water_stress'])
